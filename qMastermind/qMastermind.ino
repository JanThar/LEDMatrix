...
// switch plasmaBrightness between 0 (off) and 200 (normal)
boolean fadeIn = true;
long fadePause = 10000;
long fadeRamp = 5;
long timeStamp = millis();

void loop() {
  if (fadeIn && plasmaBrightness <200 && millis()>timeStamp+fadeRamp) {
    plasmaBrightness++;
    timeStamp = millis();
  } else if (fadeIn && plasmaBrightness == 200 && millis()>timeStamp+fadePause) {
    fadeIn = false;
    timeStamp = millis();
  } else if (!fadeIn && plasmaBrightness >0 && millis()>timeStamp+fadeRamp) {
    plasmaBrightness--;
    timeStamp = millis();
  } else if (!fadeIn && plasmaBrightness == 0 && millis()>timeStamp+fadePause) {
    fadeIn = true;
    timeStamp = millis();
  }
  runPlasma();
